      ...
      <div className="mx-auto max-w-content px-4 sm:px-6">
        ...
        <blockquote className="reveal mx-auto mt-8 max-w-2xl rounded-3xl border border-charcoal/10 bg-white p-8 text-center shadow-sm">
          ...
          <div className="mt-5 flex items-center justify-center gap-2">
            <button onClick={() => setI((i - 1 + TESTIMONIALS.length) % TESTIMONIALS.length)} className="rounded-full border border-charcoal/15 px-4 py-2 text-sm font-semibold hover:bg-charcoal hover:text-white" aria-label="Previous review">←</button>
            <span className="text-sm text-graphite/60">{i + 1} / {TESTIMONIALS.length}</span>
            <button onClick={() => setI((i + 1) % TESTIMONIALS.length)} className="rounded-full border border-charcoal/15 px-4 py-2 text-sm font-semibold hover:bg-charcoal hover:text-white" aria-label="Next review">→</button>
          </div>
        </blockquote>
      </div>
    </section>
